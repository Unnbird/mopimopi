...
check('rdpsTotal = amount - taken + given', v.rdpsTotal, '14996560');
check('adpsTotal = amount - single', v.adpsTotal, '15545248');
check('ndpsTotal = amount - taken', v.ndpsTotal, '14996560');
check('cdpsTotal = amount - single + given', v.cdpsTotal, '15545248');
...
check('owner rDPS numerator uses the folded total', s.rdpsTotal, String(1500000 - 75000 + 120000));
...
check('pet rows carry no rDPS of their own', demi.rdpsTotal, '0');
...
check('rdpsTotal for the healer', me.rdpsTotal, String(5538352 - 106862 + 985383));
const eos = c['Eos (YOU)'];
...
check('Limit Break rdpsTotal = its own damage', lb.rdpsTotal, '999999');
check('Limit Break adpsTotal', lb.adpsTotal, '999999');
check('Limit Break ndpsTotal', lb.ndpsTotal, '999999');
check('Limit Break cdpsTotal', lb.cdpsTotal, '999999');
