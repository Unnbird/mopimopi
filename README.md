...
| 傷害、傷害%、命中數、爆擊/直擊/爆直、最大傷害（含技能名）、死亡、全隊 DPS | 外掛的。FFLogs 對這一列沒有量到，整組退回 ACT。標題列會顯示 `FFLogs` 或 `ACT` 說明這一場有沒有對上 |
| 治療、HPS、溢療、治療數、爆擊治療、最大治療、全隊 HPS | **ACT 的**。這版 FFLogs 解析器對玩家不記治療（它只把 NPC 與寵物標成友方），外掛因此整組送空字串，這裡就保留 ACT 的 |
| 揮擊數、miss、命中率、承受傷害/治療、盾、Last 10/30/60/180 DPS | ACT |
| 標題列時間 | 外掛送的那一場的時間（`fflogsDurationText`）。ACT 自己的在過場時會從頭算起 |
...
**規則是整組換，不是一格一格換。** 欄位分成三組：時鐘（`DURATION` / `HEALDURATION`）、傷害、治療。每組有一個代表欄位，它是空的就代表外掛沒量到這一組，整組保留 ACT 的。

一格一格換才是要避免的事：FFLogs 的 rDPS 配 ACT 的傷害，是把同一場戰鬥的兩種量法並排放在相鄰欄位，D% 也就不再加得到 100%。但**整組沒人量過**不是混 —— 那組數字只有 ACT 有。治療就是這一組：這版解析器對玩家不記治療，照單全收它的 0 之後，治療欄變 0、旁邊的盾與溢療還是 ACT 的，有效治療就成了負數。

同一條規則也讓「外掛沒在報這一場」時整張表不會歸零：那時每一欄都是空的，於是每一欄都留 ACT 的，表格還是一張表格。FFLogs 不認得的列（NPC、兩邊名字拼法不同的）同理。已經被解析器折進主人的寵物（Carbuncle、Eos）則是 **0 而不是空的** —— 那是「算在主人身上了」，這一格要是退回 ACT 的數字，主人會被加到兩次。
...
副本內的**換場**（M8S 兩個本體之間那種）ACT 會判定脫戰、另開一個 encounter，FFLogs 則整場當一場。表格是外掛的、跟著 FFLogs 的那一場，所以整場的數字不會斷；但 ACT 那一半會在過場時從頭來過，頁面有兩處要跟著處理：

- **標題列的時間**用外掛送的 `fflogsDurationText`，不是 ACT 的 `duration`。否則標題寫 00:42、底下的表格講的是十四分鐘。
- **列**取自 ACT 當下那個 encounter 的成員，新 encounter 一開始一個人都沒有，要等各自打出第一下才會重新出現 —— 表格因此會空掉再一列一列長回來。`carryOverRows()` 在 `fflogsFightId` 沒變的期間把 ACT 暫時忘掉的列補回上一則訊息的內容：同一場裡，剛剛還在的人不是離隊，是 ACT 還沒把他記回去。換一場（fight id 變了）就不補，所以新的一拉照樣是乾淨的。

ACT 自己記的欄位（揮擊數、miss、承受傷害、Last 10/30/60/180 DPS）還是會在過場時歸零重算 —— 那些數字本來就只有 ACT 有，而 ACT 的 encounter 真的重開了。
...
- **這一場沒對上時，整張表回到 ACT 的數字**（外掛每一欄都送空的，每一欄就都留 ACT 的），只有 rDPS 家族是空的 —— 那本來就只有 FFLogs 有。標題列的 `ACT` 就是在講這件事。
- 這版 FFLogs 解析器對玩家**不記治療**，治療欄位因此一律是 ACT 的。
...
- 副本內的**換場**（M8S 兩個本體之間那種）ACT 會判定脫戰、另開一個 encounter，所以歷史紀錄會多出一列。這不是錯誤 —— 後面那一列帶的是整場（P1+P2）的數字，因為 FFLogs 把整場當一場。GCD 欄位也跟著整場走，不會在換場時歸零。表格與標題列怎麼撐過那一段，見〈過場〉。
